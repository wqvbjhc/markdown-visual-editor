...
pinned: false
license: mit
short_description: Markdown 实时预览 + 多平台导出（公众号 / 头条号 / 飞书 / 移动端）
---

# Markdown 可视化编辑器

一个基于 React + TypeScript 的两栏式 Markdown 实时预览编辑器。
左侧编写 Markdown，右侧实时预览，支持 `默认 / 公众号 / 头条号 / 飞书 / Mobile` 五种格式，并支持复制格式化内容、按当前预览格式导出 PDF，以及一键「创建飞书文档」（原生 block 结构 + 公式 + 图片 + Mermaid，需 Workers 后端）。
...
- 多平台预览：默认、公众号、头条号、飞书、Mobile 五种模式
- 图片与视频：支持 Markdown 图片、扩展媒体指令、HTML video、本地媒体会话预览
- 一键复制：按当前目标格式复制 HTML 内容
- PDF 导出：按当前预览结果导出 PDF
- 飞书（两种路径）：①「复制」按钮 + 飞书格式 → 粘进飞书文档（公式认 LaTeX 源码，任意环境可用）；②「创建飞书文档」按钮 → 原生 block 文档（需 Workers 后端，纯静态部署自动隐藏）
- 深色 / 浅色主题：支持切换并自动保存偏好
- 配色方案：支持预设配色和自定义强调色
- 去 AI 味：可选地对文本做额外清理
...
| 控件 | 作用 |
|------|------|
| `默认 / 公众号 / 头条号 / 飞书 / Mobile` | 切换当前预览格式 |
| `图片` | 插入远程图片 URL 或本地图片文件 |
| `视频` | 插入远程视频 URL 或本地视频文件 |
| `图片目录` | 授权本地目录批量载入相对路径图片（仅当前会话，刷新需重选） |
| `复制` | 复制当前格式对应的内容（飞书格式下生成飞书粘贴友好 HTML） |
| `导出 PDF` | 将当前预览内容导出为 PDF |
| `创建飞书文档` | 把当前 Markdown 转成飞书云文档（原生 block + 公式 + 图片 + Mermaid）。**仅 Workers 环境显示**，无后端部署（HF / 纯静态）自动隐藏 |
| `去 AI 味` | 对文本做额外清理 |
| `调色` | 切换预设配色或自定义强调色 |
| 主题按钮 | 切换浅色 / 深色主题 |
...
| 飞书 | 飞书文档（复制粘贴） | 公式输出 LaTeX 源码 `$...$`（飞书粘贴自动识别），标题公式降级纯文本，含公式嵌套列表自动拍平 |
| Mobile | 手机端阅读效果检查 | 使用手机外框展示内容，适合检查移动端布局 |

### 创建飞书文档（需 Workers 后端）

工具栏右侧「创建飞书文档」按钮把当前 Markdown 转成**飞书云文档**（docx block 原生结构，非剪贴板粘贴），并在新标签打开。

> **环境要求**：此按钮仅在 **Cloudflare Workers 环境**显示（前端探测 `/api/feishu/status` 返 JSON 才渲染）。**HF Spaces / 纯静态部署无后端，按钮自动隐藏**——这些环境改用「飞书」格式 +「复制」按钮（见下节）。判别依据是响应 `Content-Type`（防 SPA 假 200 误判）。
...
无 Workers 后端时（HF Spaces / 纯静态部署）的主路径，也是「创建飞书文档」按钮隐藏时的飞书出路：

1. 工具栏格式切换到 `飞书`
2. 点 `复制`
3. 到飞书文档中粘贴

**公式处理**（经实测确认飞书粘贴认 LaTeX 源码）：
- 行内 `$...$`、块级 `$$...$$` 从 KaTeX 反提原始 LaTeX，飞书粘贴**自动转公式**（非手动）
- 标题内的公式降级为纯文本（飞书标题不支持公式识别，留 `$...$` 会显示字面量）
- 公式 `<code>` 不带样式（带 background 的 code 飞书当代码字面量，不触发公式识别）

**嵌套列表公式**（飞书粘贴硬限制）：
- 飞书认扁平列表公式 + 嵌套列表**父项**公式，**不认嵌套列表子项**公式
- 故含公式的嵌套列表会**自动拍平**成单层，用 `padding-left` 模拟原层级缩进；纯文本列表保持嵌套不动

**Mermaid 图表**：复制时自动渲染成 PNG 位图 `<img>`（飞书/公众号/头条粘贴都不认 SVG，PNG base64 三平台均认）。预览里看到的矢量 SVG 只活在预览 DOM，复制走的是 PNG。

**图片 / 视频**：飞书粘贴不支持导入，公网图保留 src 碰运气，本地图/相对图转占位提示（复制时提示「N 张图需手动插入」）。

推荐使用方式：
- 写作和校对阶段：优先用 `默认`
- 发公众号前：切到 `公众号` 后再复制或导出
- 发头条号前：切到 `头条号` 后再复制或导出
- 发飞书文档：有后端用「创建飞书文档」按钮；无后端切 `飞书` 格式 + 「复制」
- 检查移动端效果：切到 `Mobile`
...
- Mermaid 图表复制时自动渲染成 PNG 位图（公众号 / 头条号 / 飞书都不认 SVG 粘贴，PNG base64 三平台均认）
...
| SVG 不支持 | 头条号剥内联 SVG。Mermaid 复制时已自动转 PNG 位图（避开此限制），但其它手写 SVG 不保留 |
...
构建产物输出到：`dist/client/`（SPA）+ `dist/markdown_visual_editor/`（Worker bundle）。`@cloudflare/vite-plugin` 启用后产物不在 `dist/` 根。
...
项目含 `Dockerfile` + 顶部 YAML frontmatter（`sdk: docker`），可作为 Docker SDK Space 部署。**飞书「创建飞书文档」按钮自动隐藏**（HF 无 Worker 后端，前端探测 `/api/feishu/status` 拿不到 JSON）；飞书复制粘贴路径（`飞书` 格式 + 「复制」）仍可用。README 必须 UTF-8 无 BOM，否则 frontmatter 解析失效。

### 部署到纯静态服务器（无飞书功能）

仅部署 `dist/` 到静态服务器，「创建飞书文档」按钮**自动隐藏**（无 Worker 后端），飞书复制粘贴路径（`飞书` 格式 + 「复制」）仍可用，其余（预览 / 复制 / PDF）正常：
...
| `pnpm build` | 构建生产版本 |
| `pnpm preview` | 构建后用 wrangler 本地预览（8787，**飞书 OAuth 不通**，redirect_uri 是 5173） |
